...
function createUser($db, $data) {
    ...
    if (empty($data['name']) || empty($data['email']) || empty($data['password'])) {
        sendResponse("Name, email, and password are required fields.", 400);
    }
    ...
    $stmt->execute();
    if ($stmt->fetch()) {
        sendResponse("Email is already registered to another user.", 409);
    }
...
function updateUser($db, $data) {
    ...
    //       If not, call sendResponse() with HTTP 400.
    if (empty($data['id'])) {
        sendResponse("User ID is required for updating.", 400);
    }
    $updateId = (int)$data['id'];

    // TODO: Look up the user by id. If not found, call sendResponse() with HTTP 404.
$stmt = $db->prepare("SELECT id FROM users WHERE id = :id");
    $stmt->bindValue(':id', $updateId, PDO::PARAM_INT);
    $stmt->execute();
    if (!$stmt->fetch()) {
        sendResponse("User not found.", 404);
    }
...
$fields = [];
    ...
    if (empty($fields)) {
        sendResponse("No fields provided to update.", 200); 
    }
...
function changePassword($db, $data) {
    ...
    $stmt->bindValue(':id', $passId, PDO::PARAM_INT);
    $stmt->execute();
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    if (!password_verify($data['current_password'], $row['password'])) {
        sendResponse("Unauthorized: Incorrect current password.", 401);
    }
